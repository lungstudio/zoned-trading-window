...
void CheckAndCreateOpenOrders(TRADE_TYPE type, double &prices[])
{
  ...
  for (int i = 0; i < ArraySize(tradablePrices); i++)
  {
    int ticketNumber = OrderSend(Symbol(), cmd, Stack, tradablePrices[i], 0, 0, 0);
    if (ticketNumber == -1) {
...
void OnTick()
{
  ...
  CheckAndCreateOpenOrders(STOP, stopEntryOrderPrices);

  // check & create limit orders
  CheckAndCreateOpenOrders(LIMIT, limitOrderPrices);
